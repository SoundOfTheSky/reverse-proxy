...
function getOption(name: string, shortName?: string) {
	...
	if (shortName && index === -1) {
		index = process.argv.indexOf(shortName);
	}
...
const rules = Object.fromEntries(process.argv.filter(argument => /^\/.+?\/\w*?=.*/.test(argument)).map(element => {
...
reverseProxy({
	verbose: true,
	port: Number.parseInt(getOption('--port', '-p') ?? '443', 10),
	rules,
	ca: getOption('--ca'),
	cert: getOption('--cert'),
	key: getOption('--key'),
